...
Generates density profiles for as many of these as you put in

example:
python prof_comparison.py halo1_particles.txt halo1_params.txt halo2_particles.txt halo2_params.txt
"""
...
res = 128 #what is the most efficient resolution? probably 128 or 256

for prof in range(0,2):
    res=128
    paramfile = open(sys.argv[2+(2*prof)],"rb") # contains particle mass and radius
    ...
    particles = []
    with open(sys.argv[1+(2*prof)]) as particlefile:
...
#            bins[part_bin] += 1
    ...
    plt.errorbar(np.log10(log_spaced_radii*scale),np.log10(true_density), yerr=sigma,label=sys.argv[1+(2*prof)].split('_')[0])
...
plt.title('Density Profile: EMDE Cut Ratio 20 vs. 40')
plt.show()
